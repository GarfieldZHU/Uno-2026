...
1. 页面壳加载后，`App` 创建 `UnoGame(seed, profile)`。
2. `UnoGame::new` 构造确定性的 108 张牌、使用 seed 洗牌、发四手牌，并以数字牌
   作为首张弃牌。
3. 门面返回 JSON `Snapshot`；AI 的具体手牌被隐藏，只公开数量。
4. 人类命令（`play_card`、`draw`、`call_uno`）由 Rust 校验。
5. AI 回合通过 `ai_step` 推进，UI 只添加短暂延迟以便看清动作；选择本身在 Rust 中
   确定性完成。
6. 终局快照以 `status: "Won"` 和 `winner` 标识获胜者。
