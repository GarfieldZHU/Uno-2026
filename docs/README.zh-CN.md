...
| 规则 | 108 张牌、离线 3–10 个席位（默认四人）、经典功能牌、变色、UNO 罚牌 |
| AI | `garfield1993-ai-simple`、`garfield1993-ai-hard`，以及两个 `uno-2026` 版本 |
| UI | 中文默认的低干扰主菜单（开始游戏/设置/关于）、统一深绿色绒布/金色视觉系统、响应式 React 牌桌、SVG 卡牌、3.8 秒发牌与起始玩家提示、桌面半透明可悬停方向箭头、摸牌/出牌/洗牌动画、结算结果层、弃牌历史和英文切换 |
| 离线设置 | 一名人类加 AI 席位；每个 AI 停顿 1–30 秒，默认三秒 |
| 联机 | Rust 房间服务 + WebSocket 快照：创建/加入/退出、主动退出或断线 AI 接管、房主开始、AI 席位、等待房间四位码、独立人类回合倒计时、全断线 3 分钟保护、token 恢复和权威下家标识 |
...
首次打开会进入中文主菜单。点击“设置”打开抽屉，可选择 3–10 名玩家（默认四人）、AI 档位，并为每个 AI
...
联机弹窗默认进入“加入房间”，只显示昵称和四位房间码；切换到“创建房间”后才显示房主专用的总席位、AI 数量和人类回合倒计时设置。创建房间由服务端生成房间码，因此创建界面不会要求填写房间码。牌局开始后有效 WebSocket 会取消房间过期倒计时；全部断线时保留 3 分钟用于恢复。服务端会接管断线玩家的座位，使用同一个 token 重连会交还控制权。
浏览器刷新后如果发现未完成牌局，会主动显示“重新连接”或“取消并清除”提示；取消会删除本地恢复凭据，重新连接会继续使用原玩家 token。牌局结束后恢复凭据和废止 WebSocket 会被清理。
