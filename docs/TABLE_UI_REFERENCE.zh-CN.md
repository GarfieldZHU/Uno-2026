...
资源加载完成后会先播放约 3.8 秒的发牌动画，再提示起始玩家并启用操作。出牌方向改为牌桌绒布上的半透明连接式 SVG 箭头环，悬停箭头段会提高对比度；中央方向胶囊不再渲染。牌局结束时显示包含获胜者姓名的胜负结算层。动画由状态驱动（`deal`、`shuffle`、`draw`、`play`、结算），并遵守 `prefers-reduced-motion`。
