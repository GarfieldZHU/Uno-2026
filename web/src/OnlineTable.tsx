    ...
    <main className="game-layout">
      <section className="table-column"><div className="table-heading"><div><p className="eyebrow">{language === "zh" ? "房间" : "ROOM"} / {room.code} · {text.seats(snapshot.players.length)}</p><h1>{snapshot.status === "Won" ? text.tableComplete : currentPlayer?.id === humanId ? text.makeYourMove : text.thinking(currentPlayer?.name ?? "AI")}</h1></div><div className="round-meta"><span>{text.turn} {String(snapshot.turn_number).padStart(2, "0")}</span><span className="direction-mark">{snapshot.direction === 1 ? "↻" : "↺"}</span></div></div>
        <div className={`felt-table table-scene ${animation ? `is-online-${animation}` : ""}`} data-animation={animation ?? undefined}>
          <div className="table-grid-lines" /><div className="table-scene-badge"><span className="live-pip" />{snapshot.current_player === humanId ? text.yourMove : text.thinking(currentPlayer?.name ?? "AI")}</div><div className="table-seats">{snapshot.players.filter((player) => player.id !== humanId).map((player, index) => { const visible = Math.min(8, Math.max(0, player.hand_count)); const center = visible > 0 ? (visible - 1) / 2 : 0; const active = player.id === snapshot.current_player; return <div key={player.id} className={`seat-player player-row seat-${SEAT_LAYOUTS[snapshot.players.length]?.[index] ?? `seat-${index}`} ${active ? "is-active" : ""}`}><span className="seat-avatar-wrap"><span className={`seat-avatar seat-avatar-${player.id % 4}`} />{active && <span className="seat-turn-pip" aria-label={language === "zh" ? "当前回合" : "current turn"} />}</span><div className="seat-player-info"><strong>{player.name}</strong><span>{player.kind.startsWith("human") ? `${player.hand_count} ${language === "zh" ? "张牌" : "cards"}` : `AI · ${player.hand_count} ${language === "zh" ? "张牌" : "cards"}`}</span></div><span className="seat-card-fan" aria-hidden="true" style={{ "--fan-total": visible, "--fan-center": center } as CSSProperties}>{Array.from({ length: visible }, (_, cardIndex) => <img key={cardIndex} src="/assets/cards/reference/card-back.svg" alt="" style={{ "--fan-index": cardIndex } as CSSProperties} />)}<b>{player.hand_count}</b></span></div>; })}<div className={`seat-player player-row seat-south seat-human ${human.id === snapshot.current_player ? "is-active" : ""}`}><span className="seat-avatar-wrap"><span className="seat-avatar seat-avatar-human" />{human.id === snapshot.current_player && <span className="seat-turn-pip" aria-label={language === "zh" ? "当前回合" : "current turn"} />}</span><div className="seat-player-info"><strong>{human.name}</strong><span>{language === "zh" ? "你 / 人类" : "YOU / HUMAN"}</span></div></div></div>
